...
#maxlength=100000
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force; try{[Net.ServicePointManager]::SecurityProtocol=[Net.ServicePointManager]::SecurityProtocol -bor 3072}catch{}; $ProgressPreference='SilentlyContinue'; $out=Join-Path $env:TEMP 'Invoke-ForensicAnalysis.ps1'; Invoke-RestMethod -Uri 'https://raw.githubusercontent.com/monobrau/mytools/main/ForensicInvestigator/Invoke-ForensicAnalysis.ps1?v=3.1.3' -OutFile $out; & $out -OutputPath "C:\SecurityReports"
# NOTE: One zip plus .sha256 under C:\SecurityReports. Loose CSVs, EventLogs, and Artifacts folders are deleted after the zip is written. Prefer elevated. No VirusTotal in this snippet.
...
#timeout=600000
#maxlength=100000
Set-ExecutionPolicy -ExecutionPolicy Bypass -Scope Process -Force; try{[Net.ServicePointManager]::SecurityProtocol=[Net.ServicePointManager]::SecurityProtocol -bor 3072}catch{}; $ProgressPreference='SilentlyContinue'; $out=Join-Path $env:TEMP 'Invoke-ForensicAnalysis.ps1'; Invoke-RestMethod -Uri 'https://raw.githubusercontent.com/monobrau/mytools/main/ForensicInvestigator/Invoke-ForensicAnalysis.ps1?v=3.1.3' -OutFile $out; & $out -OutputPath "C:\SecurityReports" -SkipEventLogs
