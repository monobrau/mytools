...
| Id | What it targets |
| --- | --- |
| `AskToolbar` (default) | Ask.com / Ask Toolbar / APN folders, ARP, HKLM/HKCU roots, leftover tasks/services, IE search scopes, Run keys, Chromium/Firefox `ask.com` prefs |
...
To add a family: copy the `AskToolbar` hashtable in `Get-PupCatalog`, change `Id` / paths / regexes. Keep patterns specific — do not use a vendor-generic word that hits unrelated software. Domain matches need a lookbehind (`(?<![A-Za-z0-9])ask\.com`) so a site like `hydroflask.com` is not flagged.
